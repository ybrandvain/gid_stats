...
-   *Conduct simple math in R:* Use `R` for addition, subtraction, multiplication etc.!  
-   *Assign variables into working memory.* 
-   *Use R functions* to conduct more sophisticated analyses.*   
-   *Write R scripts* to make our calculations reproducible.*   
-   *Consider common R mistakes* to make prepare us for our learning journey.*   
:::
...
A powerful feature of `R` is that you can create objects and then do things with these objects. [$^*$ **An `R` object** is any type of data stored in R.]{.column-margin} We first introduce an object as a simple number, but objects can be just about anything -- a map, a genome, a vector etc...
...
**Assignment in `R`:** As introduced in the previous section, we create `R` objects with the assignment operator `<-`. So, for example to assign the number one to the object, `x`, we type `x<-1`. We can then perform simple math on this object. For example, in the right margin we show that typing `2 * x` returns `2`.
...
There is so much to know about understanding variable assignment, what to name variables, and understanding working memory. Below I provide three guides for assigning, variables and understanding working memore. I suggest "unhinding" their content by clicking on them, and reading what we have to say. But, more importantly, I suggest getting out there and trying to code and read code. Most coding lessons are learned by trying, paying attentiion and trying again, rather than reading.

::: {.callout-note collapse="true"}
## **Why use the assignment operator `<-` rather than the equals sign `=`?**
...
2.  **Rare cases where it matters:** There are vary few cases where `R` will give you surprising results if you use `=` for assignment. Better to just never do this, so you don't waste days of your life chasing down some obscure error.
:::

::: {.callout-tip collapse="true"}
## **Tips for naming things (in R) -- Rules and Best practices**
...
#| echo: false
#| column: margin
#| fig-cap: "Think before you name." 
#| fig-alt: "A mocking sponge bob meme. The text says baDoBJecTnaMe."  
include_graphics("images/badName.png")
```

**There are also some "best practices" for naming objects in `R`. These make your life easier**

-   Use short names. (e.g. `height` rather than `height_measured_in_inches`).\
-   Use descriptive names. (e.g. `height` and `weight` rather than `foo` or `x`).\
-   Adopt a consistent style. camelCase or snake_case etc... Feel free to consult a style guide ([this one, for example](https://jef.works/R-style-guide/#file-names)) but most importantly be consistent.\
-   Check your spelling. I am embarrassed to admit the number of times that `mean(height)` returned [`Error: object 'height' not found`]{style="color:blue"} because I accidentally assigned heights to the variable `heigth`. These errors are super frustrating and take forever to debug.
:::

::: {.callout-tip collapse="true"}
## **UnderstandingR\`'s Working memory**
...
-   To remove everything from working memory type `rm(list = ls())`
    ...
    -   But be careful not to delete important things from your `R`, or others.
...
#| echo: false
#| column: margin
#| fig-cap: "An image of a grape leaf."
#| fig-alt: "An image of a grape leaf."
...
R has many useful built in functions. These range from standard math ([`mean()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/mean.html), [`sd`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/sd.html), etc) to statistical operations ([`t.test()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/t.test.html)) and beyond. Functions are the workhorses of R, encapsulating logic to perform specific tasks. They save time and ensure consistency across your analyses.

**Functions take arguments that we put inside parentheses.**

-   We already used the [`rm()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/rm.html) function to remove the object `z` a function when we typed `rm(z)`. Here the object, `z`, was the argument.\
-   We can similarly find the square root of 25 as `sqrt(25)`. Here the function is [`sqrt()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/MathFun.html) and the argument is `25`.\
-   We can also apply functions to vectors. or example `mean(c(1, 5, 15))` returns `r mean(c(1, 5, 15))`. Here, the function is [`mean()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/mean.html) and the argument is the vector `c(1, 5, 15)`.
...
Say you want to know the log of the square root of twenty five. You can find by

-   Nesting (i.e. functions inside of functions)

```{r}
# The log of square root of 25  Rounded to 3 digits, Nested
round(log(sqrt(25)), digits = 3)
```

-   Piping with the pipe operator `|>` [**You may see a tidyverse pipe `%>%`**. Before base `R` had a pipe tidyverse users built their own. You will see a lot of code that uses the old tidyverse pipe. The two styles of pipe behave very similarly, so don't sweat it.]{.column-margin}
...
`R` has numerous built-in datasets which you can use as a playground for practicing your skills. They offer a diverse range of data types and structures to explore. To see all the data sets available, tyye `help(package = "datasets")`.

Let's look at one simple data set, `islands`, which contains rhe areas in thousands of square miles of the landmasses which exceed 10,000 square miles. Type [`help(islands)`](https://rdrr.io/r/datasets/islands.html) to learn more. Because I dont want to see the whole thing, I will see its first few entries with the [`head()`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/head.html) function

```{r}
head(islands, n = 5)  # Example dataset
```

Use the [`length()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/length.html) function to find the number of elements in a vector
...
When coding in `R` (or any language) you are bound to make mistakes. Mistakes happen, even to the best pogrammers. Don't let them discourage you! Below I present common mistakes that R beginners and even experts make often. Click the mistakes to reveal a more detailed explanation.

::: {.callout-warning collapse="false"}
## Common `R` mistakes: Incomplete commands
...
### Common `R` mistakes: Entering the wrong thing 
...
The problem with coding is that computers always do what you tell them to. Sotypos can really hurts us. I provide a few examples of this mistake below.

::: {.callout-warning collapse="true"}
## Common `R` mistakes: C0nfu5ing 13tt3r5 for num63r5
...
::: {.callout-warning collapse="true"}
## Common `R` mistakes: Wrong.connectors
...
::: {.callout-warning collapse="true"}
## Common `R` mistakes: Things in the wrong order
...
### Watch out --  "It worked" does not mean "it will work"  
...
In this chapter we learned how to do simple math in R, how to assign variables, write/save scripts, and use functions.  As we now know how to do some simple stuff in R. Next we will investigate the different types of data in R, and  in the next chapters we will introduce the data types and data structures that allow us to do even more with R as we transition towards doing useful stuff in R.  
:::
